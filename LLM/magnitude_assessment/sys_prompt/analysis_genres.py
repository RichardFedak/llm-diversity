...
step1_sys_prompt = """
You are a helpful assistant. Your goal is to analyze the diversity in three lists of movies. For each movie, the title and its genres are provided.
...
Even if the lists are very similar or same you MUST choose the order from least to most diverse and keep the output format.
...
with open("evaluation_summary_genres_filtered.log", 'w') as summary_log:
...
print(f"Summary of evaluation logged to: evaluation_summary_genres_filtered.log")
