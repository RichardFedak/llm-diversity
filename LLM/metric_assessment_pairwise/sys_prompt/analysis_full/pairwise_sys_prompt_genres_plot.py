...
step1_sys_prompt = """
You are a helpful assistant. Your goal is to analyze the diversity in two lists of movies. For each movie, the title, genres, and plot are provided.
...
1. Focus on Genres:
   ...
   - Identify how diverse the genres are within each list and across both lists.
...
step2_sys_prompt = """
You are an evaluator given an analysis of two movie lists. Your task is to determine which list is more diverse based on the provided analysis.
Even if the lists are very similar you MUST choose one that is more diverse.
