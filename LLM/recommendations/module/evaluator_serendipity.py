...
class MovieEvaluator:
    ...
    def _generate_selected_list_info(self, list_data):
        list_info = []
        for movie in list_data:
            title = movie["title"]
            list_info.append(f"TITLE: {title}" + "\n")
        return "\n".join(list_info)
    ...
    def evaluate_data(self, data, results_folder="./results"):
        """Evaluates movie data, logs results to a JSON file, and prints summary."""
        json_log_file = f"{results_folder}/{self.evaluation_name}.json"
        ...
        for idx, item in enumerate(data):
            ...
            gold_serendipity_score = item["serendipity_score"]
            prompt = ""
            ...
            if existing_results and existing_results[idx]["serendipity_score"] == "X":
                ...
                for _, iteration in item["iterations"].items():
                    ...
                    selected_items = iteration['selected_items']
                    movies_info = self._generate_list_info(list_data)
                    selected_movies_info = self._generate_selected_list_info(selected_items)
            ...
            try:
                ...
                if all(key in output for key in [
                    ...
                    valid_outputs += 1
                    predicted_score = output["likert_answer"]
                ...
                updated_results.append({
                    ...
                    "correct": correctness if "likert_answer" in output else False,
