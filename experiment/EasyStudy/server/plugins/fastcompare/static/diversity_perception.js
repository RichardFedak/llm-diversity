...
window.app = new Vue({
  ...
  methods: {
    getRatingLabel(pair) {
      if (pair.version === "attention_check") {
        if (pair.plotSim === 1 && pair.genreSim === 1) {
          return "Please select No diversity. This is an attention check.";
        } else if (pair.plotSim === 0 && pair.genreSim === 0) {
          return "Please select High diversity. This is an attention check.";
        }
      }
      return "How diverse are these movies?";
    },
    isAttentionCheck(pair) {
      return pair.version === "attention_check";
    },
    submit() {
      this.submitted = true;
